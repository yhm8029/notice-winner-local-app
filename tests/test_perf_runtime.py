...
class PerfRuntimeTests(unittest.TestCase):
    ...
    def test_log_google_sheets_admin_duration_logs_warning_for_slow_event(self) -> None:
        with patch("backend.perf_runtime.GOOGLE_SHEETS_ADMIN_PERF_LOGGER.info") as info_mock, patch(
            "backend.perf_runtime.GOOGLE_SHEETS_ADMIN_PERF_LOGGER.warning"
        ) as warning_mock:
    ...
    def test_log_google_sheets_admin_duration_logs_info_for_fast_event_when_debug_enabled(self) -> None:
        with patch("backend.perf_runtime.GOOGLE_SHEETS_ADMIN_PERF_LOGGER.info") as info_mock, patch(
            "backend.perf_runtime.GOOGLE_SHEETS_ADMIN_PERF_LOGGER.warning"
        ) as warning_mock:
            log_google_sheets_admin_duration(
    ...
    def test_log_google_sheets_admin_duration_skips_fast_event_when_debug_disabled(self) -> None:
        with patch("backend.perf_runtime.GOOGLE_SHEETS_ADMIN_PERF_LOGGER.info") as info_mock, patch(
            "backend.perf_runtime.GOOGLE_SHEETS_ADMIN_PERF_LOGGER.warning"
        ) as warning_mock:
            log_google_sheets_admin_duration(
