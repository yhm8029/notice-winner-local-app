...
from .login_audit_logs import LoginAuditLogRepositoryError
from .login_audit_logs import LoginAuditLogRow
from .supabase_http import request_json

LOGIN_AUDIT_LOG_SELECT = ",".join(
    (
        "id",
        "organization_id",
        "user_id",
...
class SupabaseLoginAuditLogRepositoryConfig:
    base_url: str
    api_key: str
    timeout_seconds: float = 10.0

    @classmethod
    def from_env(cls) -> "SupabaseLoginAuditLogRepositoryConfig":
        ...
        if not base_url:
            raise LoginAuditLogRepositoryConfigError("SUPABASE_URL is required")
        if not api_key:
            raise LoginAuditLogRepositoryConfigError(
        ...
        if raw_timeout:
            try:
                ...
                raise LoginAuditLogRepositoryConfigError("SUPABASE_HTTP_TIMEOUT_SECONDS must be numeric") from exc
        return cls(base_url=base_url.rstrip("/"), api_key=api_key, timeout_seconds=timeout_seconds)


class SupabaseLoginAuditLogRepository(LoginAuditLogRepository):
    def __init__(self, config: SupabaseLoginAuditLogRepositoryConfig) -> None:
        self._config = config
    ...
    def create_log(
        ...
        *,
        ...
        if not isinstance(rows, list) or not rows:
            raise LoginAuditLogRepositoryError("Supabase did not return the created audit log")
        return dict(rows[0])
    ...
    def _request_json(
        ...
        *,
        ...
        return request_json(
            ...
            headers=headers,
            payload=payload,
            error_cls=LoginAuditLogRepositoryError,
        )
